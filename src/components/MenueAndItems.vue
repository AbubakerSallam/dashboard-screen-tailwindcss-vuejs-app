...
<template>
    ...
    <div class="m-2">
        <div v-for="(option, localIndex) in options" :key="uniqueIndex(localIndex)">
            <div class="flex items-center gap-2 py-0.5 px-2 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700 transition delay-100 duration-300 hover:scale-102"
                @click="toggle(localIndex)">
                <span class="font-bold text-slate-400 dark:text-slate-300" :class="
          ...
          ">
            ...
            <Transition name="collapse">
                <div v-if="option.children && currentOpenIndex === uniqueIndex(localIndex)">
                    <router-link v-for="(child, subindex) in option.children" :key="subindex" :to="child.to"
